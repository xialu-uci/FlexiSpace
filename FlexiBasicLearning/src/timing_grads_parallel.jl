...
func_map = Dict(
    "flexi1"      => FlexiBasicLearning.make_flexi1_func,
    "flexi1alg1" => FlexiBasicLearning.make_flexi1_alg1_func,
    "flexi1ode1" => FlexiBasicLearning.make_flexi1_ode1_func,
)
f = func_map[fname]
...
expdir = joinpath(expdir_base, diffname)
...
result = FlexiBasicLearning.fit_all_algs(
    ...
    maxiters = 3, save_parameters = true, time_grads = true,
)
...
mkpath(joinpath(expdir, "task_results"))
outfile = joinpath(expdir, "task_results", "$(diffname)_$(fname)_$(sname)_$(d)dof_$(np)np.jld2")
JLD2.save(outfile, "row", row)
